...
async function routeLoser(tx, categoryId, currentRound, currentPosition, loserId, loserName) {
    if (!loserId)
        return;
...
async function advanceWinner(tx, categoryId, currentRound, currentPosition, winnerId, winnerName, bracketType) {
    if (!winnerId)
        return;
    if (bracketType === "WINNER") {
        ...
        const totalLoserRounds = Math.max(...matches.map((m) => m.round));
        ...
        if (m.bracketType === "WINNER" && m.round > 1) {
            ...
            if (isOdd) {
                ...
                if (wr === 1) {
                    ...
                    if (!competitorAName)
                        competitorAName = idxA ? `Perdedor do Jogo ${idxA}` : "A definir";
                    if (!competitorBName)
                        competitorBName = idxB ? `Perdedor do Jogo ${idxB}` : "A definir";
                }
                else {
            ...
            if (!competitorAName)
                competitorAName = idxA ? `Vencedor do Jogo ${idxA}` : "Finalista Winner";
            if (!competitorBName)
                competitorBName = idxB ? `Vencedor do Jogo ${idxB}` : "Finalista Loser";
        }
        else if (m.bracketType === "RESET_FINAL") {
            const idxGF = matchMap.get(`GRAND_FINAL:1:1`);
            if (!competitorAName)
                competitorAName = idxGF ? `Vencedor da GF` : "Finalista A";
            if (!competitorBName)
                competitorBName = idxGF ? `Perdedor da GF` : "Finalista B";
        }
            ...
            if (!competitorAName)
                competitorAName = idxA ? `Perdedor do Jogo ${idxA}` : "Perdedor Semi 1";
            if (!competitorBName)
                competitorBName = idxB ? `Perdedor do Jogo ${idxB}` : "Perdedor Semi 2";
        }
        return {
            ...m,
            label,

...
export async function confirmRegistrationPaymentHandler(req: Request, res: Response) {
  const registration = await paymentsService.confirmRegistrationPayment(req.params.id as string, req.user!.id);
...
export async function confirmTeamPaymentHandler(req: Request, res: Response) {
  ...
  const team = await paymentsService.confirmTeamPayment(req.params.id as string, input, req.user!.id);
...
export async function checkoutRegistrationHandler(req: Request, res: Response) {
  const result = await checkoutService.createRegistrationCheckout(req.params.id as string, req.user!.id);
  res.status(200).json(result);
}

export async function checkoutTeamHandler(req: Request, res: Response) {
  const input = checkoutTeamSchema.parse(req.body);
  const result = await checkoutService.createTeamCheckout(req.params.id as string, req.user!.id, input.portion);
  res.status(200).json(result);
}
